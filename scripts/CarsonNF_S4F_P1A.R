...
global(DEM_resampled, fun = "notNA") # 7180630 cells (covers all CNF)
...
global(CNF_QMD_rast, fun = "notNA") # 5641440 cells
(5641440/7180630)*100 # 78.56469 % of CNF has QMD values

# areas with QMD > 5 inches
global(CNF_QMD_filt_rast, fun = "notNA") # 4420737
(4420737/7180630)*100 # 61.56475 % of CNF has trees > 5 in QMD

#### EVH ----
# all tree area
global(CNF_EVH_rast, fun = "notNA") # 5618557
(5618557/7180630)*100 # 78.24602 % of CNF has trees 

# trees > 10 ft area
global(CNF_EVH_filt_rast, fun = "notNA") # 5024646
(5024646/7180630)*100 # 69.975 % of CNF has trees > 10 ft

#### slope ----
# need to use resampled version (above) to get same resolution and extent
global(slope_resampled, fun = "notNA") # 6378993 cells 
